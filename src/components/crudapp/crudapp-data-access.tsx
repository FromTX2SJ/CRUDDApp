...
export function useCrudappProgram() {
  ...
  const accounts = useQuery({
    ...
    queryFn: () => program.account.jounernalEntryState.all(),
  });
...
export function useCrudappProgramAccount({ account }: { account: PublicKey }) {
  ...
  const accountQuery = useQuery({
    ...
    queryFn: () => program.account.jounernalEntryState.fetch(account),
  });

  const updateEntry = useMutation<string, Error, CreateEntryArgs>({
    mutationKey: ["journalEntryState", "update", { cluster }],
    mutationFn: async ({ title, message }) => {
  ...
  const deleteEntry = useMutation({
    mutationKey: ["journalEntryState", "delete", { cluster }],
